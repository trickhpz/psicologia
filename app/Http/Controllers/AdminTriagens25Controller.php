<?php namespace App\Http\Controllers;
	...
	class AdminTriagens25Controller extends \crocodicstudio\crudbooster\controllers\CBController {
	    ...
	    public function cbInit() {
			...
			$this->form = [];
			$this->form[] = ['label'=>'Nome do paciente','name'=>'pacientes_id','type'=>'select2','width'=>'col-sm-3','datatable'=>'pacientes,nome'];
			...
			$this->form[] = ['label'=>'Queixa principal contextualizada','name'=>'queixa','type'=>'textarea','width'=>'col-sm-5'];
			$this->form[] = ['label'=>'O(a) paciente já fez tratamento psicoterápico?','name'=>'realizaTratamentoPsico','type'=>'radio','width'=>'col-sm-1','dataenum'=>'não;sim'];
			$this->form[] = ['label'=>'Se sim, por quanto tempo?','name'=>'tempoTratamentoPsico','type'=>'text','width'=>'col-sm-2'];
			$this->form[] = ['label'=>'Há outros membros da família sob tratamento psicoterápico e /ou psiquiátrico?','name'=>'familiaRealizaTratPsico','type'=>'radio','width'=>'col-sm-1','dataenum'=>'não;sim'];
			$this->form[] = ['label'=>'Se sim, especificar nomes dos familiares (Atentar para familiares em gerações anteriores)','name'=>'nomesFamiliares','type'=>'textarea','width'=>'col-sm-3'];
			$this->form[] = ['label'=>'O(A) paciente tem hábito de se auto-medicar?','name'=>'habitoAutoMedicar','type'=>'radio','width'=>'col-sm-1','dataenum'=>'não;sim'];
			$this->form[] = ['label'=>'Se sim, por quanto tempo?','name'=>'tempoAutoMedicacao','type'=>'text','width'=>'col-sm-2'];
			$this->form[] = ['label'=>'Em que circunstâncias e qual tipo de medicamento utilizado?','name'=>'medicamentoUtilizado','type'=>'textarea','width'=>'col-sm-3'];
			$this->form[] = ['label'=>'O paciente faz uso de medicação psiquiátrica?','name'=>'medicacaoPsiquiatrica','type'=>'radio','width'=>'col-sm-1','dataenum'=>'não;sim'];
			$this->form[] = ['label'=>'Se sim, qual(is)?','name'=>'medicamentoPsiquiatrico','type'=>'textarea','width'=>'col-sm-3'];
			$this->form[] = ['label'=>'Há quanto tempo?','name'=>'tempoMedicamentoPsiquiatrico','type'=>'text','width'=>'col-sm-2'];
			$this->form[] = ['label'=>'Identificar o último Médico Psiquiatra','name'=>'ultimoMedicoPsiquiatra','type'=>'text','width'=>'col-sm-3'];
			$this->form[] = ['label'=>'Há histórico de internação psiquiátrica?','name'=>'historicoInternacao','type'=>'radio','width'=>'col-sm-1','dataenum'=>'não;sim'];
			$this->form[] = ['label'=>'Se sim, quando?','name'=>'quandoHistoricoInternacao','type'=>'text','width'=>'col-sm-2'];
			$this->form[] = ['label'=>'O tratamento psiquiátrico foi concluído?','name'=>'tratamentoConcluido','type'=>'radio','width'=>'col-sm-1','dataenum'=>'não;sim'];
			$this->form[] = ['label'=>'Qual o nome da Instituição de internação?','name'=>'nomeInstituicaoInternacao','type'=>'text','width'=>'col-sm-2'];
			$this->form[] = ['label'=>'O(a) paciente faz uso abusivo de drogas?','name'=>'usoAbusivoDrogas','type'=>'radio','width'=>'col-sm-1','dataenum'=>'não;sim'];
			$this->form[] = ['label'=>'Se sim, Quais as substâncias psicoativas utilizadas?','name'=>'substanciaPsicoativa','type'=>'textarea','width'=>'col-sm-3'];
			$this->form[] = ['label'=>'Atividade Profissional','name'=>'tipoAtividadeProfissional','type'=>'select','width'=>'col-sm-3','dataenum'=>'Ativo;Licenciado;Aposentado por invalidez;Desempregado'];
			$this->form[] = ['label'=>'Há caso de crianças e adolescentes','name'=>'temCriancaAdolescente','type'=>'radio','width'=>'col-sm-1','dataenum'=>'não;sim'];
			$this->form[] = ['label'=>'Período em que estuda','name'=>'periodoEstudo','type'=>'select','width'=>'col-sm-3','dataenum'=>'Matutino;Vespertino;Noturno'];
			$this->form[] = ['label'=>'Nome da escola','name'=>'nomeEscola','type'=>'text','width'=>'col-sm-3'];
			$this->form[] = ['label'=>'Horários disponíveis para a psicoterapia','name'=>'horarioDisponivel','type'=>'time','width'=>'col-sm-2'];
			$this->form[] = ['label'=>'Síntese do Caso','name'=>'sinteseCaso','type'=>'textarea','width'=>'col-sm-6'];
			$this->form[] = ['label'=>'Encaminhamento sugerido pela Equipe de Triagem A SER PREENCHIDA PELO ESTAGIÁRIO','name'=>'encaminhamentoSugerido','type'=>'radio','width'=>'col-sm-2','dataenum'=>'Pode aguardar o fluxo normal;URGENTE'];
			$this->form[] = ['label'=>'Motivo da urgência','name'=>'motivoUrgencia','type'=>'select','width'=>'col-sm-3','dataenum'=>'Afirma ideação suicida;Identifica-se ideação suicida;Possui histórico de ideação suicida;Possui histórico de tentativa de autoextermínio;Possui histórico de violência familiar contínua ;Uso abusivo de drogas psicoativas;Sem autonomia para auto-cuidado;Desempenho no trabalho se mantém instável;Transtornos mentais associados;Nível de estresse significativo nos últimos dias/meses;Tem se sentido ameaçado por situações de risco de morte;Tratamentos psiquiátricos não concluídos;Tratamento psiquiátrico em andamento;Manifesta necessidade de psicoterapia mais de uma vez por semana'];
			$this->form[] = ['label'=>'Indicação da Modalidade de atendimento','name'=>'tipoModalidadeAtendimento','type'=>'radio','width'=>'col-sm-4','dataenum'=>'Individual;Grupo*;Familiar'];
			$this->form[] = ['label'=>'Data de realização da Triagem','name'=>'dataTriagem','type'=>'date','width'=>'col-sm-2'];
			$this->form[] = ['label'=>'Nome do Estagiário Responsável pela Triagem','name'=>'alunos_id','type'=>'select2','width'=>'col-sm-3','datatable'=>'alunos,nome'];
			$this->form[] = ['label'=>'Andamento em','name'=>'andamento','type'=>'date','width'=>'col-sm-2'];
			$this->form[] = ['label'=>'Andamento','name'=>'tipoAndamento','type'=>'radio','width'=>'col-sm-1','dataenum'=>'Psicoterapia;Plantão Psicológico'];
			$this->form[] = ['label'=>'Supervisor','name'=>'coordenadores_id','type'=>'select2','width'=>'col-sm-3','datatable'=>'coordenadores,nome'];
			$this->form[] = ['label'=>'Ciente da Equipe Técnica','name'=>'cienciaEquipe','type'=>'text','width'=>'col-sm-3'];
